...
import { Zap, Smartphone, BarChart3, Shield, ArrowRight, CheckCircle } from "lucide-react";

const OPERATORS = [
  { name: "Superonline", color: "#0097a7", note: "Yetkili Bayi B9613" },
  { name: "Turkcell", color: "#FFD700" },
...
const FEATURES = [
  { icon: BarChart3, title: "Paket Karşılaştırma", desc: "Fiber, kablosuz ve ADSL paketlerini filtrele, yan yana karşılaştır.", href: "/paket-karsilastir", cta: "Paketi Bul" },
  { icon: Smartphone, title: "Mobil Tarifeler", desc: "Turkcell, Vodafone ve Türk Telekom faturalı hat fiyatlarını karşılaştır.", href: "/mobil-tarifeler", cta: "Tarifeyi Seç" },
  { icon: Zap, title: "Hız Testi", desc: "Mevcut bağlantı hızını ölç, ISP performansını gör.", href: "/hiz-testi", cta: "Hızı Test Et" },
];
  ...
  return (
    <div>
      ...
      <section className="bg-gradient-to-br from-[#1a237e] to-[#0097a7] text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-blue-100 mb-8 max-w-2xl mx-auto">
            Superonline, Turkcell, Vodafone ve Türk Telekom internet ve mobil paketlerini
            tarafsız olarak karşılaştırın. Hız testi yapın.
          </p>
        ...
        <div className="max-w-4xl mx-auto flex flex-wrap justify-center items-center gap-6 md:gap-10">
          {OPERATORS.map((op) => (
            <div key={op.name} className="flex flex-col items-center gap-1">
              ...
              {op.note && <span className="text-[10px] text-[#0097a7] font-medium">{op.note}</span>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-4 py-16 grid grid-cols-1 md:grid-cols-3 gap-6">
        {FEATURES.map((f) => (
          <div key={f.title} className="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-md transition-shadow flex flex-col">
      ...
      <section className="max-w-5xl mx-auto px-4 pb-16">
        <div className="bg-gradient-to-r from-[#0097a7] to-[#00acc1] rounded-2xl p-8 text-white flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="text-xl font-bold mb-1">Superonline Başvurun</h2>
            <p className="text-sm text-cyan-100">
              Yetkili bayi olarak en iyi kurulum hizmetini ve fiyatı sağlıyoruz.
              Ücretsiz altyapı sorgusu için bizi arayın.
            </p>
          </div>
          <a
            href="tel:+90"
            className="bg-white text-[#0097a7] font-semibold px-6 py-3 rounded-xl hover:bg-cyan-50 transition-colors whitespace-nowrap flex-shrink-0"
          >
            Şimdi Ara
          </a>
        </div>
